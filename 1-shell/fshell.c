...
int main(void){
	...
	sprintf(path,"\033[32m\033[1m*fshell*:%s\033[0m$",Fpwd()); /*打印当前路径提示*/
        cmdstring=readline(path); /*读入输入信息，打印当前路径*/
	add_history(cmdstring); /*将输入命令存入历史命令中*/
	cmdstring[strlen(cmdstring)]='\n'; /*不加换行，调用eval后识别不到换行，无法执行*/
        eval(cmdstring);
    }
    return 0;
}
